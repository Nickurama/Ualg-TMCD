...
def get_eigenvalues_1_by_1(A: np.ndarray) -> List[float]:
    return [A[0,0]]

def get_eigenvalues_2_by_2(A: np.ndarray) -> List[float]:
    a = A[0,0]
    b = A[0,1]
    c = A[1,0]
    d = A[1,1]
    ...
    quoefficients = [quo_a, quo_b, quo_c]
    roots = np.roots(quoefficients)
...
# calculates the eigenvalues for a 3x3 matrix
def get_eigenvalues_3_by_3(A: np.ndarray) -> List[float]:
    # encode the 9 values in characters from a-i
    a = A[0,0]
    b = A[0,1]
    c = A[0,2]
    d = A[1,0]
    ...
    # get the quotients for each lambda
    # lambda^3
    quo_a = -1
    # lambda^2
    quo_b = a + e + i
    # lambda^1
    quo_c = -a*i -e*i -a*e +f*h +d*b +g*c
    # lambda^0
    quo_d = a*e*i - f*h*a - d*b*i + d*c*h + g*b*f - g*c*e
    # these terms were mathematically calculated for 3x3
    # matrices via laplace method for determinant over the
    # first column of the matrix A - (delta)I.

    # get the roots (the eigenvalues) by using numpy.roots()
    quoefficients = [quo_a, quo_b, quo_c, quo_d]
    roots = np.roots(quoefficients)

    # convert the roots (eigenvalues) to floats
    eigenvalues: List[float] = []
    for root in roots:
        eigenvalues.append(float(root))

    # return the eigenvalue list
    return eigenvalues

def get_eigenvalues_3_by_3_or_below(A: np.ndarray) -> List[float]:
    if A.shape[0] == 3:
        return get_eigenvalues_3_by_3(A)
    elif A.shape[0] == 2:
        return get_eigenvalues_2_by_2(A)
    elif A.shape[0] == 1:
        return get_eigenvalues_1_by_1(A)
    else:
        raise Exception("Matrix should be 3x3 or below.")
...
def norm_squared(vector: np.ndarray) -> float:
    # the squared norm is the sum of all the elements of the vector, squared.
    # which is the equivalent of the dot product of the vector with itself
    return float(np.dot(vector, vector))

# computes the norm of a vector
def norm(vector: np.ndarray) -> float:
    # which is the square root of the norm squared
    return np.sqrt(norm_squared(vector))

# Takes the transpose of A (for efficiency), the current iteration i (zero index), the transpose of Q and the R arrays, and the previous computed values (v)
# and does one gram-schmidt iteration for the given i.
def gram_schmidt_iteration(A_t: np.ndarray, i: int, Q_t: np.ndarray, R: np.ndarray, v: np.ndarray) -> None:
    # first iteration
    if i == 0:
        # the first v is just u
        v[i] = A_t[i]
        # the norm of the first v
        v_norm = norm(v[i])
        # updates the diagonal of R for the first v, which is just the norm of v
        R[i][i] = v_norm
        # calcualtes the inverse of the norm
        inv_norm = 1 / v_norm
        # calculates the Q for the first iteration, which is just
        # A's first column multiplied by the inverse of the norm
        Q_t[i] = inv_norm * A_t[i]
        # the first iteration is over
        return

    # NOTE: u is just the current column / vector
    # NOTE: each v is the orthogonal vector transformation for the equivalent space, generated by the gram-schimdt formula

    # the current column / vector
    u = A_t[i]
    # the current v, starts as just u
    curr_v = u
    # the gram-schmidt formula then does the subtraction of the previous v
    # each multiplied by the dot product of themselves by u, divided by it's norm squared
    for k in range(i):
        # the previous v
        iter_v = v[k]
        # the squared norm of the previous v
        iter_v_norm_squared = norm_squared(iter_v)
        # the quotient that will be multiplied by the previous v, as per the gram-schmidt formula
        quotient = (np.dot(u, iter_v)) / (iter_v_norm_squared)

        # the norm of the previous v
        iter_v_norm = norm(iter_v)
        # the R at the given k, for this column i is given by the dot product of u by the previous v,
        # divided by the norm. Since we already calculated this but with the norm squared (the quotient), we multiply by the norm
        R[k][i] = quotient * iter_v_norm

        # we update curr_v iteratively, subtracting each previous v with the quotient (as such applying the subtraction of each previous v)
        curr_v = curr_v - (quotient * iter_v)
    
    # the previous value "i" is calculated in the curr_v
    v[i] = curr_v

    # the current v norm
    curr_v_norm = norm(curr_v)
    # the R at the diagonal is just the norm of that column's v
    R[i][i] = curr_v_norm

    # the inverse of the current v's norm
    inv_norm = 1 / curr_v_norm
    # the Q at the given column is the inverse of the norm multiplied by v (the orthogonal vector)
    Q_t[i] = inv_norm * curr_v


# does the QR decomposition of matrix A
def qr_decomposition(A: np.ndarray) -> Tuple[np.ndarray, np.ndarray]:
    # the minimum size of A's columns/rows
    min_size = min(A.shape[0], A.shape[1])
    # Q will have the same shape as A
    Q = np.zeros(shape=A.shape)
    # R will be a square matrix with the minimum size of A
    R = np.zeros(shape=[min_size, min_size])


    # A and Q transposed
    A_t = A.T
    Q_t = Q.T
    # initialize v (equivalent orthogonal vectors) empty
    v = np.empty(A_t.shape)
    # performs each iteration of the gram-schmidt algorithm
    for i in range(A.shape[1]):
        gram_schmidt_iteration(A_t, i, Q_t, R, v)
    # updates Q with the values of the updates Q transposed
    Q = Q_t.T
